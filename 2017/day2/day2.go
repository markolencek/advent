...
func main() {
	...
	for _, line := range lines {
		...
		for _, single := range split {
			tmpNum, err := strconv.Atoi(single)
			if err != nil {
				panic(err.Error())
			}
			numLine = append(numLine, tmpNum)
		}
		sort.Ints(numLine)
		nums = append(nums, numLine)
		tmp := numLine[len(numLine)-1] - numLine[0]
		csums = append(csums, tmp)
		csum += tmp
	}
